...
class Sam3BasePredictor:
    ...
    def start_session(
        ...
        import inspect as _inspect_pred
        _init_sig = set(_inspect_pred.signature(self.model.init_state).parameters.keys())
        init_kwargs = dict(resource_path=resource_path, offload_video_to_cpu=offload_video_to_cpu)
        if "offload_state_to_cpu" in _init_sig:
