...
class FusedMaskEncoder(MaskEncoder):
    ...
    def forward(
        ...
        **kwargs,
        ...
        pix_feat = pix_feat.to(self.pix_feat_proj.weight.dtype)
        x = self.pix_feat_proj(pix_feat)
        x = x + masks
        x = self.fuser(x)
        x = x.to(next(self.out_proj.parameters()).dtype) if list(self.out_proj.parameters()) else x
        x = self.out_proj(x)
...
class SequenceGeometryEncoder(nn.Module):
    ...
    def _encode_points(self, points, points_mask, points_labels, img_feats):
        ...
        if self.points_direct_project is not None:
            points = points.to(self.points_direct_project.weight.dtype)
        ...
        if self.points_pool_project is not None:
            ...
            sampled = sampled.to(self.points_pool_project.weight.dtype)
        ...
        if self.points_pos_enc_project is not None:
            ...
            enc = enc.to(self.points_pos_enc_project.weight.dtype)
    ...
    def _encode_boxes(self, boxes, boxes_mask, boxes_labels, img_feats):
        ...
        if self.boxes_direct_project is not None:
            boxes = boxes.to(self.boxes_direct_project.weight.dtype)
        ...
        if self.boxes_pos_enc_project is not None:
            ...
            enc = enc.to(self.boxes_pos_enc_project.weight.dtype)
    ...
    def forward(self, geo_prompt: Prompt, img_feats, img_sizes, img_pos_embeds=None):
        ...
        if self.final_proj is not None:
            final_embeds = final_embeds.to(self.final_proj.weight.dtype)
